...
class PhysicsInformedNN:
    ...
    def __init__(self, Z_theta, Z_RE, Z_T, Z_heat, theta, T, layers_theta, layers_k, layers_psi, layers_T, layers_lam, lb, ub):
        ...
        #初始化拉格朗日算子
        with tf.variable_scope('lamda_'):
            self.lamda = tf.Variable([10.0], dtype=tf.float32)
        ...
        # self.theta_pred = self.net_theta(self.z_tf, self.t_tf, self.weights_theta, self.biases_theta)
        ...
        self.loss_T = tf.reduce_mean(tf.square(self.T_tf - self.T_pred))*self.adaptive_constant_tf
        # self.loss_T = self.loss_T1*self.adaptive_constant_tf
        ...
        self.loss_1 = self.loss_theta*tf.Variable([10.0], dtype=tf.float32)  + self.loss_RE
        self.loss_2 = self.loss_heat + tf.reduce_mean(tf.square(self.T_tf - self.T_pred))*tf.Variable([100.0], dtype=tf.float32)
        #learning rate
        ...
        self.learning_rate2 = tf.train.exponential_decay(learning_rate=0.0005, global_step=tf.Variable(0, trainable=False), decay_steps=5000, decay_rate=0.99, staircase=True)
        
        self.heat_gr = tf.gradients(self.loss_heat,self.weights_T)[0]
        self.T_gr = tf.gradients(self.loss_T,self.weights_T)[0]
        self.max_heat_gr= tf.reduce_max(tf.reduce_max(tf.abs(self.heat_gr)))
        self.mean_T_gr= tf.reduce_mean(tf.reduce_mean(tf.abs(self.T_gr))) 
        self.adaptive_constant = self.max_heat_gr / self.mean_T_gr   
        
        

        # L-BFGS-B method----------------------------------------------------------------------------------------------------------------------------------
        self.optimizer = tf.contrib.opt.ScipyOptimizerInterface(self.loss_1,
                                                        var_list = RRE_vars,
                                                        method = 'L-BFGS-B',
        ...
        self.train_op_Adam = self.optimizer_Adam.minimize(loss=self.loss_1, var_list=RRE_vars)
        ...
        # optimizer for heat----------------------------------------------------------------------------------------------------------------------------------
        self.optimizer_heat = tf.contrib.opt.ScipyOptimizerInterface(self.loss_2,
                                                        var_list = heat_vars,
                                                        method = 'L-BFGS-B',
                                                        options = {'maxiter': 50000,
                                                                   'maxfun': 50000,
        ...
        self.optimizer_Adam_heat = tf.train.AdamOptimizer(learning_rate=self.learning_rate2)
        self.train_op_Adam_heat = self.optimizer_Adam_heat.minimize(loss=self.loss_2, var_list=heat_vars)
    ...
    def net_k(self, X, weights, biases): #tanh, exp
    ...
    def net_lam(self, X, weights, biases): #tanh, exp
        num_layers = len(weights) + 1
        H = X
        for l in range(0, num_layers - 2):
            W = weights[l]
        ...
        b = biases[-1]
        Y = tf.add(tf.matmul(H, W), b)
        lam = tf.exp(Y)  # force psi to be positive
        return lam
        
    def net_psi(self, z, t, weights, biases): #tanh, -exp
        X = tf.concat([z, t],1)
        num_layers = len(weights) + 1
    ...
    def net_theta(self, X, weights, biases): #tanh, 输出层无特别激活函数,sigmoid
        num_layers = len(weights) + 1
        H = X
        for l in range(0, num_layers - 2):
            W = weights[l]
    ...
    def net_f_heat(self, z, t): #loam
        b1 = tf.constant(1.56728, dtype=tf.float32)
        ...
        psi_z = tf.gradients(psi, z)[0]
        flux = -k*(psi_z + 1.0)
        lam = (b1 + b2*theta + b3*tf.sqrt(theta+eps)) * mult + beta*Cw*tf.abs(flux) #10 16  10 14
    ...
    def net_f_heat_net(self, z, t): #loam
        ...
        # lam = (b1 + b2*theta + b3*tf.sqrt(theta+eps)) * mult + beta*Cw*tf.abs(flux) #10 16  10 14
        log_h = tf.math.log(-psi)
        # lam = self.net_lam(-log_h, self.weights_lam, self.biases_lam) #几百级别的 500-1000 用神经网络拟合整个热导
        lam1 = self.net_lam(-log_h, self.weights_lam, self.biases_lam) #几级别的
    ...
    def train(self, N_iter):
        ...
        for it in range(N_iter):
            ...
            if it % 10 == 0:
                ...
                loss_value = self.sess.run(self.loss_1, tf_dict)
    ...
    def train_for_T(self, N_iter):      # ANN
        start_time = time.time()
        tf_dict = {self.z_tf: self.z, self.t_tf: self.t, self.T_tf: self.T}
        # Adam
        for it in range(N_iter):
    ...
    def train_heat_ld(self, epoch):      
        ...
        for i in range(epoch):
            for j in range(len(self.z)):
                tf_dict = {self.z_tf: self.z[j, None], self.t_tf: self.t[j, None], self.theta_tf: self.theta[j, None]}
                self.sess.run(self.train_op_Adam_heat, tf_dict)
                if j % 100 == 0:
                    elapsed = time.time() - start_time
                    loss_value = self.sess.run(self.loss_2, tf_dict)
                    print('Epoch: %d, it: %d, Loss: %.3e, Time: %.2f' %(i, j, loss_value, elapsed))
                    start_time = time.time()     
            # 更新lambda
            tf_dict = {self.z_tf: self.z, self.t_tf: self.t, self.theta_tf: self.theta}
            loss_value = self.sess.run(self.loss_heat, tf_dict)
            self.lamda = self.lamda + Sk * loss_value
            print('Epoch: %d, it: %d, Loss: %.3e, Time: %.2f' %(i, j, loss_value, elapsed))        
        # L-BFGS-B
    ...
    def train_heat(self, N_iter):      
        ...
        for it in range(N_iter):
            tf_dict = {self.z_heat_tf: self.z_heat, self.t_heat_tf: self.t_heat, self.T_tf: self.T, 
                       ...
                       self.T_t_tf: self.T_t_star, self.adaptive_constant_tf: self.adaptive_constant_val}
            self.sess.run(self.train_op_Adam_heat, tf_dict)
            if it % 10 == 0:
                elapsed = time.time() - start_time
...
if __name__ == "__main__":
    ...
    layers_k = [1, 10, 1]
    layers_theta = [1, 10, 1]
    layers_T = [2, 40, 40, 40, 40, 1] #四层网络
    layers_lam = [1, 20, 1]

    # data
    data = pd.read_csv("D:/PINNs_RRE-master/article_data/change/case321_siltloam.csv")
    ...
    # noise
    print(np.std(T_star))
    noise_theta = noise*np.random.randn(theta_star.shape[0], theta_star.shape[1])#*np.std(theta_star) # standard normal distibuiton with 0 mean and stndard error of the noise value
    theta_noise = theta_star + noise_theta
    noise_T = noise*np.random.randn(T_star.shape[0], T_star.shape[1])*np.std(T_star) # standard normal distibuiton with 0 mean and stndard error of the noise value
    ...
    T_pred, T_t_pred = model.predict_T(Z_star)
    dataset = pd.DataFrame({'z': z_star.flatten(), 't': t_star.flatten(),
                        'T_actual': T_star.flatten(), 'T_pred': T_pred.flatten(),
                        'T_t_pred': T_t_pred.flatten()})
    dataset.to_csv("D:/PINNs_RRE-master/article_case/3_2_change/case1/T_siltloam_noise2.csv")
    ...
    dataset.to_csv("D:/PINNs_RRE-master/article_case/3_2_change/case1/RE_siltloam_noise2.csv")
    
    loss_list, lam_list, f1_list, f2_list, f3_list = model.train_heat(50000)
    theta_pred, psi_pred, K_pred, f_pred, theta_t_pred, psi_z_pred, psi_zz_pred, K_z_pred, flux_pred, h_pred, T_pred, lam_pred = model.predict(Z_star)
    dataset = pd.DataFrame({'z': z_star.flatten(), 't': t_star.flatten(),
                    'theta_actual': theta_star.flatten(), 'theta_pred': theta_pred.flatten(),
                    'T_actual': T_star.flatten(), 'T_pred': T_pred.flatten(),
                    ...
                    'psi_z_pred': psi_z_pred.flatten(), 'psi_zz_pred': psi_zz_pred.flatten(),
                    'K_z_pred': K_z_pred.flatten(), 'h_pred': h_pred.flatten(), 'lam_pred': lam_pred.flatten()})
    dataset.to_csv("D:/PINNs_RRE-master/article_case/3_2_change/case1/REheat_siltloam_noise2.csv")
    ...
    lookup.to_csv("D:/PINNs_RRE-master/article_case/3_2_change/case1/lookup_siltloam_noise2.csv", index = False)                                   
