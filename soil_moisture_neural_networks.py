...
class PhysicsInformedNN:
    ...
    def __init__(self, Z_theta, Z_RE, Z_T, Z_heat, theta, T, layers_theta, layers_k, layers_psi, layers_T, layers_lam, lb, ub):
        ...
        self.f_heat, self.lam, self.f1, self.f2, self.f3= self.net_f_heat_net(self.z_heat_tf, self.t_heat_tf)
        ...
        self.loss_1 = self.loss_theta*tf.Variable([10.0], dtype=tf.float32)  + self.loss_RE  + self.loss_l2*tf.Variable([0.001], dtype=tf.float32)
        # self.loss_2 = self.loss_theta + self.loss_heat + self.loss_T
        self.loss_2 = self.loss_heat + self.loss_T*tf.Variable([100.0], dtype=tf.float32) 
        #learning rate
        ...
        # optimizer for T----------------------------------------------------------------------------------------------------------------------------------
        self.optimizer_T = tf.contrib.opt.ScipyOptimizerInterface(self.loss_T,
                                                        var_list = T_vars,
                                                        method = 'L-BFGS-B',
                                                        options = {'maxiter': 50000,
                                                                   'maxfun': 50000,
        ...
        self.train_op_Adam_T = self.optimizer_Adam_T.minimize(loss=self.loss_T, var_list=T_vars)
        ...
        self.optimizer_Adam_heat = tf.train.AdamOptimizer(learning_rate=self.learning_rate)
...
if __name__ == "__main__":
    ...
    noise_T = noise*np.random.randn(T_star.shape[0], T_star.shape[1]) 
    T_noise = T_star + noise_theta

    # fixed points
    Z_theta_train = Z_star[fixed_list,:]
    theta_train = theta_noise[fixed_list, :]
    Z_T_train = Z_star[fixed_list_T,:]
    T_train = T_star[fixed_list_T, :]
